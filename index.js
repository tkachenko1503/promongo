...
Collection.prototype.update = function() {
	var cargs = splitArgs(arguments);
	return nodeify(this
		._apply(DriverCollection.update, cargs.args)
		.then(function (result) {
			return result[1];
		}), cargs.callback);
};
...
Collection.prototype.runCommand = function(cmd, opts, callback) {
	...
	return this._get().then(function (collection) {
		...
		return nodeify(nApply(collection.db.command, [commandObject], collection.db), callback);
	});
};
	...
	return this._get().then(function (collection) {
		...
		if (collection.opts && cargs.callback) {
			...
			return nApply(fn, cargs.args, collection)
				.then(function (res) {
					collection.opts.safe = safe; return res;
				});
		}
	});
};
...
var connect = function(config, collections) {
	...
	var getdb = thunk(function () {
		return nApply(mongodb.Db.connect, [connectionString], mongodb.Db)
			.then(function (db) {
				that.client = db;
				that.emit('ready');
				db.on('error', function(err) {
